...
useHead({
  title: 'Edit Menu Item - Admin Panel'
});
...
// Get menu item ID from route
const menuItemId = computed(() => {
...
const fetchParentMenuItems = async () => {
  try {
    ...
    // Filter out the current menu item and its children to prevent circular references
    if (menuItemId.value) {
      parentMenuItems.value = result.items.filter(item => 
        item.id !== menuItemId.value && 
        !isChildOf(item, menuItemId.value)
      );
    } else {
...
// Check if an item is a child of the specified parent ID
const isChildOf = (item: any, parentId: number): boolean => {
  if (item.parentId === parentId) return true;
  if (item.children && item.children.length > 0) {
    return item.children.some((child: any) => isChildOf(child, parentId));
  }
  return false;
};
...
const fetchMenuItem = async () => {
  ...
  try {
    menuItem.value = await getMenuItem(menuItemId.value);
  } catch (error) {
...
const handleSubmit = async () => {
  ...
  try {
    const success = await updateMenuItem(menuItemId.value);
...
watch(selectedLanguage, (newLang, oldLang) => {
  ...
  // Fetch menu item data for the new language
  if (menuItemId.value) {
    fetchMenuItem(menuItemId.value);
  }
});
...
onMounted(async () => {
  ...
  // Fetch languages, menu item, and parent menu items
  await Promise.all([
    fetchLanguages(),
    fetchMenuItem()
  ]);
  
  // After fetching the menu item, fetch parent menu items to exclude the current item
  await fetchParentMenuItems();
...
<template>
  <div class="min-h-screen bg-slate-50">
    <div class="flex-1 overflow-y-auto">
      <form v-if="menuItem" @submit.prevent="handleSubmit" class="space-y-6">
        <!-- Header -->
        <PageHeader
          title="Edit Menu Item"
          :description="`Editing: ${menuItem?.translations?.[0]?.label || 'Menu Item'}`"
        >
          <template #actions>
            <!-- Language Switcher -->
      ...
      <!-- Loading State -->
      <div v-else-if="isLoading" class="bg-white dark:bg-neutral-800 shadow-sm rounded-lg p-6">
        <div class="animate-pulse space-y-4">
          <div class="h-8 bg-gray-200 dark:bg-neutral-700 rounded w-1/4"></div>
      ...
      <!-- Not Found State -->
      <div v-else class="bg-white dark:bg-neutral-800 shadow-sm rounded-lg p-6 text-center">
        <div class="mx-auto h-12 w-12 text-gray-400">
          <svg class="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
        </div>
        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">Menu item not found</h3>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">The menu item you are looking for does not exist or has been deleted.</p>
        <div class="mt-6">
          <NuxtLink
            to="/menu-items"
